...
session_start();
    ...
    $conn=mysqli_connect('localhost','root','', 'onlinecrimemanagement');
    if(!$conn)
    {
        die("could not connect".$conn->connect_error);
    }
    //mysqli_select_db("onlinecrimemanagement",$conn);
...
if(isset($_POST['s'])){
    $con=mysqli_connect('localhost','root','', 'onlinecrimemanagement');
    if(!$con)
    {
        die("could not connect : " .$conn->connect_error);
    }
    ...
    if($var>=0)
    {
      ...
      //mysqli_select_db("onlinecrimemanagement"); 
...
<head>
	...
	<link href="../style/complainer_page1.css" rel="stylesheet" type="text/css" media="all" />
